...
def run_tractor_preflight(
    *, config_root: str | None = None, dataset_root: str | None = None,
    ...
    try:
        ...
        with (config_path / "robot" / "hunter_se_improved.yaml").open() as stream:
            robot = (yaml.safe_load(stream) or {})["robot"]
        if abs(float(robot["collision_radius_m"]) - model.footprint_radius_m) > 1e-9:
            report.errors.append("TRACTOR footprint differs from hunter_se_improved collision radius")
