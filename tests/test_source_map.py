"""Authoritative check for docs/SOURCE_MAP.md's "verbatim copy" claims
...
SOURCE_MAP.md's table is a human-readable summary of these assertions.

`drl_agent` is explicitly NOT a package.xml dependency of this package (see
SOURCE_MAP.md's header) -- it is a sibling ROS package in the same
...
if not _DRL_AGENT_ROOT.is_dir():
    pytest.skip("drl_agent sibling package not present in this checkout -- SOURCE_MAP.md "
...
# docs/SOURCE_MAP.md, geometry.py deliberately excluded (see below).
VERBATIM_COPY_PAIRS = [
    ...
    ("hunter_kinodynamic_rl/rl/networks/tqc.py", "rl/networks/tqc.py"),
...
def test_verbatim_copy_pairs_are_still_byte_identical(new_rel, source_rel):
    ...
    assert new_path.is_file(), f"SOURCE_MAP.md references a missing file: {new_path}"
    assert source_path.is_file(), f"SOURCE_MAP.md references a missing drl_agent file: {source_path}"
    assert _sha256(new_path) == _sha256(source_path), (
        f"{new_rel} has drifted from drl_agent's {source_rel} -- SOURCE_MAP.md's 'Verbatim copies' "
...
# (as SOURCE_MAP.md's "Adapted" row now claims) even though geometry.py as a
...
def test_geometry_py_is_no_longer_claimed_verbatim_but_shared_functions_are_byte_identical():
    ...
    finer-grained claim SOURCE_MAP.md's 'Adapted' table row now makes, and
    ...
    assert not whole_file_identical, (
        ...
        "removed or geometry_utils.py gained an equivalent, update SOURCE_MAP.md to move this row "
    ...
    for fn_name in _GEOMETRY_SHARED_FUNCTIONS:
        assert hasattr(theirs, fn_name), (
            ...
            "_GEOMETRY_SHARED_FUNCTIONS/SOURCE_MAP.md need updating"
        )
        our_src = inspect.getsource(getattr(ours, fn_name))
        their_src = inspect.getsource(getattr(theirs, fn_name))
        assert our_src == their_src, (
            f"geometry.py's {fn_name}() has diverged from geometry_utils.py's -- SOURCE_MAP.md's "
    ...
    assert not hasattr(theirs, "to_world_frame"), (
        "drl_agent's geometry_utils.py now ALSO defines to_world_frame() -- SOURCE_MAP.md's claim that "
